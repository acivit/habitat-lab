...
def parse_args():
    ...
    parser.add_argument('--use-gpt-human', type=lambda x: x.lower() == 'true', 
                        default=True,
                        help='Use GPT for human simulation, Llama if false (True/False, default: True)')
    
    parser.add_argument('--start-logic-human', type=lambda x: x.lower() == 'true',
                        default=True,
                        help='Restart human simulation or read from saved results (True/False, default: True)')
    ...
    parser.add_argument('--max-scenes', type=int, default=5,
                        help='Maximum number of scenes to process (sd > X, default: 5)')
    parser.add_argument('--max-profiles', type=int, default=10,
                        help='Maximum number of profiles to process (i > X, default: 10)')
...
# CUDA_VISIBLE_DEVICES="0" python coopera_main/human_sim/human_sim.py --use-gpt-human True --start-logic-human True --collab-type 2
...
if __name__ == "__main__":
    ...
    # Restart human simulation or not
    start_logic_human = args.start_logic_human
        ...
        for i, (profile_string, big_five) in enumerate(zip(profile_string_list, big_five_list)):
            ...
            for d, day in enumerate(days):
                ...
                for j, time_ in enumerate(times):
                    ...
                    human_conversation_hist = intention_proposal_mllm(results_path, i, scene_id, [day, j, time_], [human_retrieved_intentions, human_retrieved_predicates], room_list, [profile_string, big_five], temperature_dict, model_dict, gpt=use_gpt_human, collab=collab_type, start_over=start_logic_human)
