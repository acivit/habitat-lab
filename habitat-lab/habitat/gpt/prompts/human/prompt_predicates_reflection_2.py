...
def reflect_predicates_prompt_2_1(time_, intention, sampled_motion_list, obj_room_mapping, profile_string, retrieved_memory):
    contents = f"""
    Input:
    ...
    Instructions:
    1.  Break down the intention into 3 tasks for collaboration with a robot.
    2.	Task types: 
        - Type 1: Pick a dynamic object and place oy on a fixed, static object (static objects cannot be moved).
    3.  For static objects, use only objects from the static object dict (exact name). For dynamic objects, use only objects from the dynamic object dict (exact name).
    4   Both static and dynamic objects must be specified. Importantly, they cannot be none.
...
def reflect_predicates_prompt_2_2(time_, intention, sampled_motion_list, obj_room_mapping, profile_string, retrieved_memory):
    contents = f"""
    Input:
    1.  The proposed intention: {intention} at time: {time_}.
...
def reflect_predicates_2(time_, intention, sampled_motion_list, obj_room_mapping, profile_string, retrieved_memory, output_path, existing_response=None, temperature_dict=None, 
                  model_dict=None, conversation_hist=None, collab=2, gpt=True):

    if collab == 1:
        predicates_user_contents_filled = reflect_predicates_prompt_2_1(time_, intention, sampled_motion_list, obj_room_mapping, profile_string, retrieved_memory)
    elif collab == 2:
        predicates_user_contents_filled = reflect_predicates_prompt_2_2(time_, intention, sampled_motion_list, obj_room_mapping, profile_string, retrieved_memory)
